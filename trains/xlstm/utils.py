...
class WeightDecayOptimGroupMixin(nn.Module, ABC):
    ...
    def get_weight_decay_optim_groups(
        ...
        unassigned_params = [
            up
            for up in unassigned_params
            if not hasattr(up, "requires_grad") or up.requires_grad
        ]
    ...
    def get_weight_decay_optim_group_param_names(
        ...
        def _is_in_sequence(
            param: nn.Parameter, sequence: Sequence[nn.Parameter]
        ) -> bool:
            for p in sequence:
                if param is p:
                    return True
        ...
        names_weight_decay = [
            pn for pn, p in self.named_parameters() if _is_in_sequence(p, weight_decay)
        ]
        names_no_weight_decay = [
            pn
            for pn, p in self.named_parameters()
            if _is_in_sequence(p, no_weight_decay)
        ]
